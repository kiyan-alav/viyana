...
import {TOAST_OPTIONS} from "@/utlis/Toast";
...
function NewProduct() {
    ...
    const onFileSelectProductImage = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        <>
            ...
            <div className="flex flex-col gap-4 mt-4">
                <div className="grid grid-cols-4 gap-4">
                    ...
                    />
                    <Select isRequired className="max-w-xs" label="دسته بندی" labelPlacement="outside" classNames={{
                        trigger: "bg-white data-[hover=true]:bg-white group-data-[focus=true]:bg-white"
                    }}>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <Textarea
                        ...
                        classNames={{
                            inputWrapper: "h-max bg-white data-[hover=true]:bg-white group-data-[focus=true]:bg-white"
                        }}
                        isRequired
                        label="مشخصات"
                        labelPlacement="outside"
                ...
                <div className="grid grid-cols-2 gap-8">
                    <Input type="file" id="category" className="hidden" onChange={onFileSelectProductImage}/>
                    ...
                    برای بارگذاری <span className="text-palette-50">کلیک</span> نمایید
                                </span>
                        </label>
                    </div>
                    <div className="col-span-1 flex gap-4">
                        {productImgDisplays.map((img, index) => (
                            <div key={index} className="relative border border-solid rounded-lg w-36 h-24">
                                <img src={img} alt={`category-${index}`} className="w-full h-full"/>
                                ...
                                <Input type="file" id={`replace-${index}`} className="hidden" onChange={(e) => replaceImage(index, e)}/>
                            </div>
                        ))}
                    </div>
                </div>
